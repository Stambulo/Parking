    ...
    private fun setErrorMessage(validationStateList: List<InputState>) {
        with(binding) {
            inputEmailLayout.error =
                if (validationStateList.first().isValid) {
                    null
                } else {
                    resources.getString(validationStateList.first().errorMessage)
                }

            inputPasswordLayout.error = if (validationStateList.last().isValid) {
                null
            } else {
