        ...
        with(binding) {
            states.forEach {
                when (it.fieldType) {
                    InputTextType.NAME -> {
                        if (it.isValid) {
                            hideError(inputNameLayout)
                        } else {
                            showError(inputNameLayout, it.errorMessage)
                        }
                    }
                    InputTextType.EMAIL -> {
                        if (it.isValid) {
                            hideError(inputEmailLayout)
                        } else {
                            showError(inputEmailLayout, it.errorMessage)
                        }
                    }
                    InputTextType.PHONE -> {
                        if (it.isValid) {
                            hideError(inputPhoneLayout)
                        } else {
                            showError(inputPhoneLayout, it.errorMessage)
                        }
                    }
                    InputTextType.PASSWORD -> {
                        if (it.isValid) {
                            hideError(inputPasswordLayout)
                        } else {
