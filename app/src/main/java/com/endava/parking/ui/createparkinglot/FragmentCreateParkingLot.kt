        ...
        with(binding.createParkingContainer) {
            stateList.forEach { state ->
                when (state.fieldType) {
                    InputTextType.NAME -> {
                        inputParkingNameLayout.error = if (state.isValid) {
                            null
                        } else {
                            requireContext().getString(state.errorMessage)
                        }
                    }
                    InputTextType.ADDRESS -> {
                        inputParkingAddressLayout.error = if (state.isValid) {
                            null
                        } else {
                            requireContext().getString((state.errorMessage))
                        }
                    ...
                    InputTextType.TIME -> {
                        inputParkingOpHoursLayout.error = if (state.isValid) {
                            null
                        } else {
                            requireContext().getString(state.errorMessage)
                        }
